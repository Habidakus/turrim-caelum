...
var castle1 = null
var castle0 = null
var currentPathCastleSelection : int = 0
var pathArray : Array = []
...
func get_castle() -> Area2D:
	return castle0 if currentPathCastleSelection == 0 else castle1
...
func on_mob_spawn(mobId : int, rng : RandomNumberGenerator):
	if mobId % 11 == 0:
		...
		currentPathCastleSelection = (currentPathCastleSelection + 1) % 2
		pathArray.append(generate_path(start, get_castle().position, mobId, rng))
		if increase != true:
...
func add_path_point(start: Vector2, end: Vector2, desired_length: float, curve : Curve2D, mobId: int, rng: RandomNumberGenerator) -> Curve2D:
	...
	for i in 50:
		...
		var isBad : bool = min(p.distance_to(castle0.position), p.distance_to(castle1.position)) < buffer
