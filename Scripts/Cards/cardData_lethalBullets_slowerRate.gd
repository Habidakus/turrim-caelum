...
func initialize_for_purchase(player: Player, cardData: CardData):
	...
	newBulletsPerMinute = currentBulletsPerMinute - get_delta_decreased_rate_of_fire_per_minute(cardData)

func get_description(player: Player, cardData: CardData) -> String:
	var currentBulletsPerMinute = 60 / player.rate_of_fire
	var newDamage: float = player.bullet_damage + get_damage_we_will_add(player, cardData)
	var damageText : String = str("Bullet damage increased from ", int(round(player.bullet_damage)), " to ", int(round(newDamage)))
...
func apply_card(player: Player):
	...
	player.rate_of_fire = 60 / newBulletsPerMinute
